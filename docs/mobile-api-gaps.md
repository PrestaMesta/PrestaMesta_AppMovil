...
releído para Checkpoints 3, 4 y 5).
...
8 endpoints en total, dos dominios de identidad (`clientes` / `administradores`) con audiencias
JWT distintas, sin mecanismo de logout/revocación (JWT stateless, expira via `JWT_EXPIRES_IN`,
por defecto 8h), sin refresh token.
...
| Home: próximo pago / saldo estimado | ninguno | No | Implementado (Checkpoint 5): no existe endpoint que liste préstamos del cliente ni sus pagos, así que `HomeScreen` nunca muestra saldo/próximo pago — solo un saludo real y, si existe, la última `LoanSubmissionResponse` exitosa de la sesión actual (en memoria, `SessionLoanSummaryController`). |
| Estado de solicitud (línea de tiempo) | ninguno lee solicitudes propias | No | Implementado (Checkpoint 5): el cliente puede ver el resultado de **la solicitud que él mismo acaba de enviar en esa sesión** (respuesta del POST, la misma `SessionLoanSummaryController` que Home), pero no hay forma de consultar solicitudes pasadas ni su estado actualizado (p. ej. si un admin la aprueba después). Sin una solicitud en memoria: estado vacío + aviso de limitación, nunca una línea de tiempo fabricada. |
